...
class Board:
    ...
    def end(self):
        ...
        if self.blue_fig_left==0:
            return 1
        if len(self.get_possible_moves(not self.white_turn))==0:
            return 2
        else:
            return 3
...
def minimax_a_b(eval_type ,board, depth, move_max):
    best_rating = -1000
    best_i = 0
    if not move_max:
        best_rating *= -1
...
def minimax_a_b_recurr(eval_type, board, depth, move_max, a, b):
    ...
    for move in board.get_possible_moves(move_max):
        move_board = deepcopy(board)
        move_board.make_ai_move(move)
        successors.append(move_board)
    
    if move_max:
        for possible_board in successors:
            a = max(a, minimax_a_b_recurr(eval_type, possible_board, depth-1, not move_max, a, b))
            if a >= b:
                return b
        return a
    else:
